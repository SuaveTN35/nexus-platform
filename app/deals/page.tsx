  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-primary-900">Deals Pipeline</h1>
          <p className="text-primary-600 mt-2">Manage your sales pipeline and track deals</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <Card>
          <div className="text-center">
            ...
            <p className="text-3xl font-bold text-primary-900 mt-2">{formatCurrency(totalValue)}</p>
          </div>
        </Card>
        <Card>
          <div className="text-center">
            <p className="text-sm text-primary-600">Open Deals</p>
        ...
        <Card>
          <div className="text-center">
            <p className="text-sm text-primary-600">Won Deals</p>
        ...
        <Card>
          <div className="text-center">
            <p className="text-sm text-primary-600">Total Won Value</p>
            <p className="text-3xl font-bold text-green-600 mt-2">{formatCurrency(wonValue)}</p>
          </div>
        </Card>
      </div>
        ...
        <div className="flex space-x-4 min-w-max pb-4">
          {pipelineStages.map((stage) => (
            <Card key={stage} className="min-w-[300px] flex-shrink-0" padding="md">
              <div className="mb-4">
                <h3 className="font-semibold text-primary-900">{stage}</h3>
              ...
              <div className="space-y-3">
                {dealsByStage[stage]?.map((deal) => (
                  <Link
                    key={deal.id}
                    href={`/deals/${deal.id}`}
                    className="block bg-primary-50 rounded-lg p-4 cursor-pointer hover:bg-primary-100 transition-colors"
                  >
                    <div className="font-medium text-primary-900 mb-1">{deal.name}</div>
                    <div className="text-lg font-bold text-primary-700 mb-2">
                      {formatCurrency(deal.value)}
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-primary-600">{deal.probability}% probability</span>
